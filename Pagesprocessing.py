...
def Modeextracttion(datapins,df,listdata,q):
...
def Bucketcombinefunc(devicesinput): # Get the list from each package input  
       ...
       for qw in range(0,len(devicesinput)):  # using qw to get the value in the list array 
                   ...
                   combinedictdata[str(devicesinput[qw])] = str(Packagetypebucket[qw])+","+str(Pinsbucket)  #Get the list variable to generate the json and dictionary data structure                  
...
def extractpinspackage(df,listdata,configfile): 
          ...
          #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
           ...
           for q in range(0,len(listdata)): 
                  ...
                  Modeextracttion(devicename,df,listdata,q),Modeextracttion(packagedevice,df,listdata,q),Modeextracttion(pinsnumber,df,listdata,q)  
